...
#
CY_EXAMPLE_NAME = CE222967_QSPI_FRAM_ACCESS_WITH_PSOC6_SMIF

#
# Description of the example project to display
#
CY_EXAMPLE_DESCRIPTION = This code example implements the QSPI F-RAM host controller using PSoC 6 Serial Memory Interface (SMIF) block and demonstrates accessing the Excelon Ultra QSPI F-RAM in SPI, DPI, and QPI modes. The status LED (RGB) turns green when the CE output matches with the expected output, and turns red when the CE output differs from the expected output. The code example also enables the UART interface to connect to a PC to monitor the result.
#
...
CY_APP_CM4_SOURCE =                \
    Source/main.c                  \
    Source/qspi_fram_apis.c        \
    Source/qspi_fram_apis.h        \
    Source/stdio_user.c            \
    Source/stdio_user.h            \
    readme.txt              
#
