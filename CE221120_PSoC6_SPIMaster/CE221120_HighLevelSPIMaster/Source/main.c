...
* Version: 1.0
*
* Description: This example project demonstrates SPI communication between two SCB blocks
* of PSoC 6 device (one configured as Master and another as slave). Firmware uses
* high level APIs for the master configuration and communication.
*
...
int main(void)
{
    ...
    for(;;)
    {
	    ...
	    {
	    	status = checkTranferStatus(SIZE_OF_PACKET);
	    }
