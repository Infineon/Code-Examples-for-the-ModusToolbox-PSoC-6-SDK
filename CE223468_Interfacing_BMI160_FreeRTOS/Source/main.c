...
*  a BMI160 Motion Sensor over an I2C interface within a FreeRTOS task. This
*  example reads raw motion data and estimates the orientation of the board.
*
...
/** Priority of user tasks in this application. Valid range of task priory is priority
 * is 0 to ( configMAX_PRIORITIES - 1 ), where configMAX_PRIORITIES is defined within
 * FreeRTOSConfig.h. Low priority numbers denote low priority tasks. */
#define TASK_MOTION_SENSOR_PRIORITY 	(configMAX_PRIORITIES - 1)

/* Stack size of user tasks in this project */
#define TASK_MOTION_SENSOR_STACK_SIZE	(256u)
...
*  the RTOS scheduler.
*  In this application there are two tasks:
*  1. Task_Motion : configures and processes the Motion Sensor
*  2. Task_Debug  : prints debug message via UART STDIO
*
* Parameters:
*  void
*
...
int main(void)
{
    ...
    printf("\x1b[2J\x1b[;H");
    printf("CE223468 - PSoC 6 MCU: Interfacing BMI160 (I2C) in FreeRTOS\r\n");

	/* Create the user Tasks. See the respective Task definition for more
	 * details of these tasks */
    xTaskCreate(Task_Motion, "Motion Sensor Task", TASK_MOTION_SENSOR_STACK_SIZE,
